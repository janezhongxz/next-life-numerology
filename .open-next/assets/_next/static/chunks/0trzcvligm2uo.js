...
日期：${b}`}({birthDate:a.birthDate,name:a.name,lifePath:a.lifePath,attitude:a.attitude,birthDay:a.birthDay,personalYear:a.personalYear,birthGridCounts:a.birthGridCounts,missingNumbers:a.karmicLessons,dominantNumber:a.dominantNumber,age:a.age,question:a.question,lang:a.lang||"zh"}),t=localStorage.getItem("api_provider")||"deepseek",r=localStorage.getItem("api_key")||"sk-570adfb7923a4248b65fcafa9a26d520";if(r&&"sk-570adfb7923a4248b65fcafa9a26d520"!==r){f("正在生成详细报告...");let i=await fetch(`${"deepseek"===t?"https://api.deepseek.com":"https://api.openai.com/v1"}/chat/completions`,{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${r}`},body:JSON.stringify({model:"deepseek"===t?"deepseek-chat":"gpt-4o",messages:[{role:"user",content:e}],temperature:.7,max_tokens:8e3})});if(!i.ok)throw Error(`API错误: ${i.status}`);let n=await i.json();d(n.choices[0].message.content)}else{f("正在生成详细报告...");let t=await fetch("https://api.deepseek.com/chat/completions",{method:"POST",headers:{"Content-Type":"application/json",Authorization:"Bearer sk-570adfb7923a4248b65fcafa9a26d520"},body:JSON.stringify({model:"deepseek-chat",messages:[{role:"user",content:e}],temperature:.7,max_tokens:8e3})});if(!t.ok)throw Error(`API错误: ${t.status}`);let r=await t.json();d(r.choices[0].message.content)}}catch(e){console.error("生成错误:",e),d(w(a))}finally{h(!1)}})()},[a]);let w=e=>{let t=s(e.age),r=e.personalYear?o(e.personalYear):o(1),i=e.karmicLessons.length>0?e.karmicLessons.join("、"):"无";e.lang;let n=new Date,a=`${n.getFullYear()}/${n.getMonth()+1}/${n.getDate()}`,l=e.birthDate.split("-"),u=`${l[0]}年${parseInt(l[1])}月${parseInt(l[2])}日`;return`🌟 生命数字战略蓝图：${e.name} | ${t}的${r.type}
