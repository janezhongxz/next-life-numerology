...
- 生日数：${a}
- 流年数字：${l||"未提供"}
- 天赋九宫格各数字出现次数：1出现${d[1]}次、2出现${d[2]}次、3出现${d[3]}次、4出现${d[4]}次、5出现${d[5]}次、6出现${d[6]}次、7出现${d[7]}次、8出现${d[8]}次、9出现${d[9]}次
- 最强数字：${c}
- 缺失数：${g}
- 用户年龄：${h}岁（${m}）
- 用户问题困惑：${p||"未提供"}
...
你是 ${i}号主命（LifePath=${i}），用 **${a}号生日结果导向（BirthDay=${a}）** 做系统落地，用 **${n}号洞察与价值导向（Attitude=${n}）** 建立理解深度；但你当前的"动力来自业务不够"的根因是 **黑洞缺失数 ${g}**。

${g.includes("3")?'· 3：表达没有补成"可行动/可回应"的业务结构':""}
${g.includes("4")?'· 4：执行没有补成"稳定运行的节律系统"':""}
...
你的天赋九宫格中，数字${c}出现了${Math.max(...Object.values(d))}次，这是你能量最强的核心引擎。
...
执行风险： 报告中的行动建议属于启发式引导，用户基于此做出的任何行为及后果，由用户本人承担，开发者不承担法律责任。
...
日期：${x}`}({birthDate:a.birthDate,name:a.name,lifePath:a.lifePath,attitude:a.attitude,birthDay:a.birthDay,personalYear:a.personalYear,birthGridCounts:a.birthGridCounts,missingNumbers:a.karmicLessons,dominantNumber:a.dominantNumber,age:a.age,question:a.question,lang:a.lang||"zh"}),t=localStorage.getItem("api_provider")||"deepseek",r=localStorage.getItem("api_key")||"sk-570adfb7923a4248b65fcafa9a26d520";if(r&&"sk-570adfb7923a4248b65fcafa9a26d520"!==r){f("正在生成详细报告...");let i=await fetch(`${"deepseek"===t?"https://api.deepseek.com":"https://api.openai.com/v1"}/chat/completions`,{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${r}`},body:JSON.stringify({model:"deepseek"===t?"deepseek-chat":"gpt-4o",messages:[{role:"user",content:e}],temperature:.7,max_tokens:8e3})});if(!i.ok)throw Error(`API错误: ${i.status}`);let n=await i.json();u(n.choices[0].message.content)}else{f("正在生成详细报告...");let t=await fetch("https://api.deepseek.com/chat/completions",{method:"POST",headers:{"Content-Type":"application/json",Authorization:"Bearer sk-570adfb7923a4248b65fcafa9a26d520"},body:JSON.stringify({model:"deepseek-chat",messages:[{role:"user",content:e}],temperature:.7,max_tokens:8e3})});if(!t.ok)throw Error(`API错误: ${t.status}`);let r=await t.json();u(r.choices[0].message.content)}}catch(e){console.error("生成错误:",e),u(w(a))}finally{h(!1)}})()},[a]);let w=e=>{let t=s(e.age),r=e.personalYear?o(e.personalYear):o(1),i=e.karmicLessons.length>0?e.karmicLessons.join("、"):"无",n="en"===e.lang,a=new Date,l=`${a.getFullYear()}/${a.getMonth()+1}/${a.getDate()}`,d=e.birthDate.split("-"),u=`${d[0]}年${parseInt(d[1])}月${parseInt(d[2])}日`;return`🌟 生命数字战略蓝图：${e.name} | ${t}的${r.type}
